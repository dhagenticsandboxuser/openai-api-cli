...
func TestRun(t *testing.T) {
	for _, filePrompt := range []bool{false, true} {
		t.Run(map[bool]string{false: "stdin", true: "file"}[filePrompt], func(t *testing.T) {
			...
			defer server.Close()
			args := []string{"-model", "test-model", "-credentials", keyPath, "-endpoint", server.URL, "-params-file", paramsPath, "-max-output-tokens", "10", "-project", "project"}
			if filePrompt {
...
func TestStream(t *testing.T) {
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			var output bytes.Buffer
			err := stream(strings.NewReader(c.input), &output)
...
func TestOutputFailure(t *testing.T) {
	err := stream(strings.NewReader("data: {\"type\":\"response.output_text.delta\",\"delta\":\"x\"}\n\n"), brokenWriter{})
